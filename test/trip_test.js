...
describe('trips', () => {
  ...
  before(async () => {
    ...
    const res2 = await requester
      .post('/1.0/signin')
      .send(users[1]);
    accessToken2 = res2.body.data.access_token;
  });
  ...
  it('get trip settings: user has access (user_id = 1, trip_id = 1)', async () => {
    ...
    const expectedTrip = {
      ...
      otherTrips: [{ id: 1, name: 'Paris with mom' }],
  ...
  it('get trip settings: user has access to multiple trips (user_id = 2, trip_id = 2)', async () => {
    ...
    const expectedTrip = {
      ...
      calendar_id: '775bc5c3120',
      otherTrips: [
        { id: 2, name: 'London with bao' },
        { id: 3, name: 'Bangkok with bao' },
      ],
      duration: 5,
    };
  ...
  it('get trip settings: user has no access (user_id = 1, trip_id = 2)', async () => {
    const res = await requester
      .get('/1.0/trip?id=2')
      .set('Authorization', `Bearer ${accessToken1}`);
