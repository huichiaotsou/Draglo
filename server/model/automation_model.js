...
const arrangeAutomationResult = async (tripId, userId, dayId, startDate, wholeTrip) => {
    ...
    keys.map( unixDay =>{
        wholeTrip[unixDay].map(activity => {
            ...
            let timeStart = dateForStart.setMinutes(dateForStart.getMinutes() + activity.startTime) //).setHours(dateForStart.getHours() - timezoneOffset)
            let timeEnd = dateForEnd.setMinutes(dateForEnd.getMinutes() + activity.end) //).setHours(dateForEnd.getHours() - timezoneOffset)
